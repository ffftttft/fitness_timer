...
/// Riverpod 版简单计时控制器（对应当前 TimerProvider 的能力）。
class SimpleTimerController extends StateNotifier<TimerState> {
  ...
  void _ensureTicker() {
    _ticker ??= Timer.periodic(const Duration(milliseconds: 200), (_) {
      ...
      // 秒级节流持久化：仅当 elapsed 秒数变化且为 running/paused 时写入快照
      if ((state.status == TimerStatus.running ||
  ...
  void _scheduleCurrentIntervalNotification() {
    ...
    // 计算当前 interval 内剩余时长：intervalRemaining = interval.duration - intervalElapsed
    final elapsed = state.elapsed;
    var acc = Duration.zero;
    for (var i = 0; i < index; i++) {
      acc += intervals[i].duration;
...
/// simpleTimerController 的 family Provider：按 WorkoutConfig 实例区分。
